...
sub main {
    ...
    my $russian_text = join $EMPTY, @out;

    my $encoding = term_encoding;
    say decode( $encoding, $russian_text );
...
sub translate_text {
    ...
    try {
        ...
        $out = join $EMPTY, @matches;      
    }
    catch {
        carp( q{Couldn't get url - } . $url . $res->status_line . $_ );
    }
	return $out;
...
sub translate_ru_text {
    ...
    try {
        ...
        if ( !$firemech->success() ) {
            carp(   q{Couldn't find tab PROMT in firefox, please load url:}
                  . $url
                  . $_ );
