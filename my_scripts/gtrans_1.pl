...
sub main {
	...
	my $text=$google_text;#$russian_text;
use Convert::Cyrillic;
use Lingua::DetectCharset;

my $charset = Lingua::DetectCharset::Detect($text);
say $text;
die "Unknown charset" if $charset eq 'ENG';
say $charset;
my $converted = Convert::Cyrillic::cstocs($charset, 'win', $text);	
say $converted;
    # say $russian_text;
    # my $encoding = term_encoding;
    # say encode( $encoding, $russian_text );
    return 1;
}
